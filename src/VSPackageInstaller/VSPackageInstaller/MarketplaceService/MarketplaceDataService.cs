...
namespace VSPackageInstaller.MarketplaceService
{
    internal class MarketplaceDataService
    {
        ...
        public List<ExtensionDataItem> _extensionDataItems = new List<ExtensionDataItem>();
        public List<ExtensionDataItem> _paidExtension = new List<ExtensionDataItem>();
        ...
        {
            GetMarketplaceDataItems("15.9", new List<string>() {"Pro", "Enterprise", "Community", "Ultimate", "IntegratedShell", "Premium", "TeamExplorer", "SQL"}, DateTime.MinValue, TestCallback );
        }
        ...
        private List<PublishedExtension> GetNextPageFromMarketplace(int pageNumber, int pageSize, String vsVersion, IEnumerable<string> skus)
        {
            try
            {
                ...
                request.Headers.Add("User-Agent", "VSIDE-VSPackageInstaller" + vsVersion);
